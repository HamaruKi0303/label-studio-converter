...
def convert_coco_to_ls(input_file, out_file,
    ...
    rectangles_from_name, keypoints_from_name = from_name + \
        '_rectangles', from_name + '_keypoints'
    ...
    for i, annotation in enumerate(coco['annotations']):
        ...
        if keypoints and not keypoints_once:
            logger.warning(
                'Keypoints are partially supported without skeletons')
        ...
        image_file_name, image_width, image_height = image[
        ...
        if 'bbox' in annotation:
            item = create_bbox(
                annotation, categories, rectangles_from_name, image_height, image_width, to_name)
            task[out_type][0]['result'].append(item)

        if 'segmentation' in annotation:
            item = create_segmentation(
                annotation, categories, segmentation_from_name, image_height, image_width, to_name)
            task[out_type][0]['result'].append(item)
